...
obj = cv2.imread('./pyppe/experimental/capture1.png', cv2.IMREAD_GRAYSCALE) # 찾고 싶은거
src = cv2.imread('./pyppe/experimental/capture2.png', cv2.IMREAD_GRAYSCALE) # 대상
...
_, obj_bin = cv2.threshold(obj, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
obj_bin = cv2.bitwise_not(obj_bin)
obj_bin = cv2.Canny(obj_bin, 50,100)

#cv2.imshow("test", obj_bin)
...
src_bin = cv2.Canny(src_bin, 50,100)
